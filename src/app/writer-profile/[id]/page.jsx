...
import { getUserById, getUserSession } from "@/lib/core/session";

const WriterProfilePages = async ({ params }) => {
  const { id } = await params;
  const user = await getUserById(id);
  const books = await getBooksForWriterProfilePage(id);
  console.log(user, "user from writer profile page");
       ...
       "
            >
              <div className="p-4 rounded-xl bg-gray-50">
                ...
                <p className="font-medium text-gray-800 truncate">{user?._id}</p>
              </div>
...
export default WriterProfilePages;
